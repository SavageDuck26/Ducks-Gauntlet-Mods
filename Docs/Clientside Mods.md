...
For example: Knossos uses a config (a list of settings) to tell the level builder what to assign and where. If even a single difference between the host and non-host players is present in that config, then the layout can change and the non-host player will crash.
