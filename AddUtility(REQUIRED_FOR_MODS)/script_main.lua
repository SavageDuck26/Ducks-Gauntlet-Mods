...
AddUtility.spawn_unit = function(path, spawn_at_unit) -- Spawn GENERIC UNITS at another unit.
    ...
    if short_unit_lookup and short_unit_lookup[path] then
        unit_path = short_unit_lookup[path]
    elseif string.find(path, "/") or string.find(path, "\\") then
        -- Path contains directory separators — treat as a full unit path
        -- (e.g. "units/custom/crane_test/crane" or "characters/avatar_warrior/warrior")
        unit_path = path
    else
...
AddUtility.spawn_unit_path = function(unit_path, spawn_at_unit)
    if unit_path == nil or spawn_at_unit == nil then
        print("AddUtility.spawn_unit_path: Invalid argument(s).")
        return
    end

    local world = FlowCallbacks.state_game and FlowCallbacks.state_game.world_proxy and FlowCallbacks.state_game.world_proxy:get_world()
    if not world then
        print("AddUtility.spawn_unit_path: No world available to spawn unit.")
        return
    end
    ...
    if not ok or not spawned_unit then
        print("AddUtility.spawn_unit_path: World.spawn_unit failed: " .. tostring(spawned_unit))
        return nil
    end
...
-- =================================================================================
-- Unit Apply Functions
AddUtility.apply_status_effect = function(unit, status_effect)
    if EntityAux.is_alive_entity(unit) then
        EntityAux.call_master(unit, "status_receiver", "add_status_effect", status_effect)
    end
end
-- =================================================================================
