...
Mods.hook:set_object(_G, "require", function(orig, path, ...)
    ...
    if path == "foundation/lua/entity/entity_spawner" then
        Mods.hook:set_object_path("EntitySpawner", "trigger_on_entity_registered", function(orig, self, unit, owned)
            orig(self, unit, owned)

            apply_effects(unit, owned) -- Apply Non-Destructively
    
        end, MOD_NAME .. ".EntitySpawner.trigger_on_entity_registered", MOD_NAME)
    end
